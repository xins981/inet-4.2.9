...
bool TcpConnection::isSegmentAcceptable(Packet *packet, const Ptr<const TcpHeader>& tcpseg) const
{
    ...
    uint32 len = packet->getByteLength() - B(tcpseg->getHeaderLength()).get(); // 报文的数据长度
    uint32 seqNo = tcpseg->getSequenceNo(); // 报文的序号字段
    uint32 ackNo = tcpseg->getAckNo(); // 报文的确认好字段
    uint32 rcvWndEnd = state->rcv_nxt + state->rcv_wnd; // 窗口内最后一个字节序号加一
    bool ret;
