...
{
    int socketId = getTags(msg).getTag<SocketReq>()->getSocketId();//每个消息都会携带对应的socket标识符，这句话就是读标识符
    TcpConnection *conn = findConnForApp(socketId); // 找到socket对应的tcp链接
    ...
    if (!conn->processAppCommand(msg))
        removeConnection(conn); // 收到关闭连接的命令就释放连接
}
