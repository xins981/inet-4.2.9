...
TcpEventCode TcpConnection::processSegment1stThru8th(Packet *packet, const Ptr<const TcpHeader>& tcpseg)
{
    ...
    if (tcpseg->getHeaderLength() > TCP_MIN_HEADER_LENGTH)
    {    // Header options present? TCP_HEADER_OCTETS = 20
        // PAWS
        if (state->ts_enabled) {
    ...
    if (fsm.getState() == TCP_S_SYN_RCVD || fsm.getState() == TCP_S_ESTABLISHED || fsm.getState() == TCP_S_FIN_WAIT_1 || fsm.getState() == TCP_S_FIN_WAIT_2)
    {
        ...
        if (payloadLength > 0)
        {
            ...
            if (hasEnoughSpaceForSegmentInReceiveQueue(packet, tcpseg))
            {    // enough freeRcvBuffer in rcvQueue for new segment?
            ...
            {    // not enough freeRcvBuffer in rcvQueue for new segment
